...
/** Matches `declare_id!` in `draingang/src/lib.rs`. */
...
/**
 * @param {{ confirmationStatus?: string | null }} [st]
 * @param {"processed" | "confirmed" | "finalized"} want
 */
...
/**
 * Browser-safe confirmation: avoids `Connection.confirmTransaction({ lastValidBlockHeight })`,
 * which races `signatureSubscribe`/a single probe against `getBlockHeight` polled every ~1s
 * (noisy RPC + false "block height exceeded" when WS notify never fires).
 *
 * @param {Connection} connection
 * @param {string} signature
 * @param {{ lastValidBlockHeight?: number, commitment?: "processed" | "confirmed" | "finalized", pollMs?: number, maxPollMsCap?: number }} [opts]
 */
export async function confirmSignatureHttpPolling(
  ...
  while (Date.now() - start < maxPollMs) {
    ...
    if (signatureMeetsCommitment(st, commitment)) {
      return;
    }
  ...
  if (opts.lastValidBlockHeight != null) {
    try {
      ...
      if (h > opts.lastValidBlockHeight) {
        throw new Error(
          `Transaction ${signature} was not confirmed before its blockhash expired. Try again (RPC may have dropped the transaction, or approval took too long).`,
        );
      }
    } catch (e) {
      if (e instanceof Error && e.message.includes("was not confirmed")) throw e;
    }
  }
  throw new Error(
...
export async function settle(ctx, userPubkey, opts) {
  ...
  const heightAfterSign = await connection.getBlockHeight("confirmed");
  if (heightAfterSign > lastValidBlockHeight) {
    throw new Error(
      "Transaction blockhash expired while waiting for wallet approval. Approve faster or tap Enter again.",
    );
  }
